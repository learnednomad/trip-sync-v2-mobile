...
function Skeleton({
  ...
  return (
    <View
      className={`${width} ${height} rounded bg-gray-200 ${className} animate-pulse`}
    />
  );
}
      ...
      {Array.from({ length: count }).map((_, index) => (
        ...
        >
          ...
          <View className="mb-2 flex-row items-center">
            <Skeleton width="w-4" height="h-4" className="mr-2 rounded" />
    ...
    >
      ...
      >
        <View className={isGridMode ? '' : 'flex-1'}>
          ...
          >
            <Skeleton
              width={isGridMode ? 'w-3/4' : 'w-2/3'}
              height={isGridMode ? 'h-4' : 'h-5'}
            />
          ...
          {/* Destination */}
          <View className="mb-2 flex-row items-center">
            <Skeleton width="w-3" height="h-3" className="mr-1 rounded" />
