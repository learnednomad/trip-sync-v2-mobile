...
import { X, MapPin, Calendar, Users, DollarSign, Sparkles } from '@/components/ui/icons';
...
const tripTypeOptions: Array<{ label: string; value: TripType; icon: string }> = [
...
const popularDestinations = [
  'New York', 'London', 'Tokyo', 'Paris', 'Barcelona', 'Dubai', 
  'Singapore', 'Rome', 'Sydney', 'San Francisco', 'Berlin', 'Amsterdam'
];

const commonPreferences = [
  'Budget-friendly', 'Luxury', 'Local experiences', 'Food & dining',
  'Nightlife', 'Museums', 'Nature', 'Shopping', 'Photography',
  'Relaxation', 'Adventure sports', 'Historical sites'
];

export function QuickPlanModal({ visible, onClose, onCreateTrip }: QuickPlanModalProps) {
        ...
        <View className="px-4 py-6 border-b border-gray-100">
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center">
              ...
              <Text className="text-xl font-bold text-gray-900 ml-2">Quick Plan</Text>
            </View>
            <Button
              variant="ghost"
              size="sm"
          ...
          <Text className="text-sm text-gray-500 mt-2">Step {step} of {totalSteps}</Text>
        </View>
              ...
              <View>
                ...
                <View className="mb-4">
                  ...
                  <Input
                    ...
                    onChangeText={(value) => setPlanData(prev => ({ ...prev, destination: value }))}
                    className="mb-2"
                  />
                    ...
                    <View className="flex-row space-x-2">
                      {popularDestinations.map((destination) => (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onPress={() => setPlanData(prev => ({ ...prev, destination }))}
                        >
                          <Text className="text-gray-700 text-xs">{destination}</Text>
                        </Button>
                      ))}
                    </View>
                  </ScrollView>
                ...
                <View>
                  ...
                  <View className="flex-row flex-wrap">
                    {tripTypeOptions.map((option) => (
                      <Button
                        key={option.value}
                        variant={planData.tripType === option.value ? 'default' : 'outline'}
                        size="sm"
                        onPress={() => setPlanData(prev => ({ ...prev, tripType: option.value }))}
                        className="mr-2 mb-2 flex-row items-center"
                      >
                        <Text className="mr-1">{option.icon}</Text>
                        <Text className={planData.tripType === option.value ? 'text-white text-xs' : 'text-gray-700 text-xs'}>
                          {option.label}
                        </Text>
                      </Button>
                    ))}
              ...
              <View className="space-y-4">
                <View>
                  ...
                  <DatePicker
                    ...
                    onDateChange={(date) => setPlanData(prev => ({ ...prev, startDate: date }))}
                ...
                <View>
                  ...
                  <DatePicker
                    ...
                    onDateChange={(date) => setPlanData(prev => ({ ...prev, endDate: date }))}
                    placeholder="Select end date"
                    mode="date"
                    minimumDate={planData.startDate}
                  />
                </View>
              </View>
                ...
                <View className="bg-blue-50 p-3 rounded-lg">
                  <Text className="text-blue-800 text-sm">
                    Trip duration: {Math.ceil(
                      (new Date(planData.endDate).getTime() - new Date(planData.startDate).getTime()) / (1000 * 60 * 60 * 24)
                    )} days
                  </Text>
                </View>
              )}
              ...
              <View className="space-y-4">
                <View>
                  <Text className="text-sm font-medium text-gray-700 mb-2">
                    <DollarSign width={14} height={14} color="#6B7280" /> Budget (USD)
                  </Text>
                  <Input
                    placeholder="Enter total budget"
                    value={planData.budget?.toString() || ''}
                    onChangeText={(value) => setPlanData(prev => ({ ...prev, budget: parseInt(value) || 0 }))}
                ...
                <View>
                  <Text className="text-sm font-medium text-gray-700 mb-2">
                    <Users width={14} height={14} color="#6B7280" /> Number of travelers
                  </Text>
                  <View className="flex-row space-x-2">
                    {[1, 2, 3, 4, 5, 6].map((num) => (
                      <Button
                        key={num}
                        variant={planData.participants === num ? 'default' : 'outline'}
                        size="sm"
                        onPress={() => setPlanData(prev => ({ ...prev, participants: num }))}
                        className="w-12"
                      >
                        <Text className={planData.participants === num ? 'text-white' : 'text-gray-700'}>
                          {num}
                        </Text>
                      </Button>
                    ))}
            ...
            <View className="space-y-6">
              ...
              <View className="flex-row flex-wrap">
                {commonPreferences.map((preference) => (
                  <Button
                    key={preference}
                    variant={planData.preferences?.includes(preference) ? 'default' : 'outline'}
                    size="sm"
                    onPress={() => handlePreferenceToggle(preference)}
                    className="mr-2 mb-2"
                  >
                    <Text className={planData.preferences?.includes(preference) ? 'text-white text-xs' : 'text-gray-700 text-xs'}>
                      {preference}
                    </Text>
                  </Button>
                ))}
              ...
              <View className="bg-gray-50 p-4 rounded-lg mt-6">
                <Text className="text-sm font-medium text-gray-700 mb-2">Trip Summary</Text>
                <Text className="text-sm text-gray-600">
                  📍 {planData.destination} • {planData.tripType?.toLowerCase()} trip
                </Text>
                <Text className="text-sm text-gray-600">
                ...
                {planData.preferences && planData.preferences.length > 0 && (
                  <Text className="text-sm text-gray-600">
                    ...
                    {planData.preferences.length > 3 && ` +${planData.preferences.length - 3} more`}
                  </Text>
                )}
              </View>
            </View>
