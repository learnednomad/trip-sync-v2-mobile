...
import { TripStatusManager, type TripStatus, type StatusTransition, type TripLifecycleEvent } from '../TripStatusManager';
...
const mockStatusHistory: TripLifecycleEvent[] = [
  {
    id: 'event-1',
    tripId: 'trip-1',
    fromStatus: null,
...
describe('TripStatusManager Component', () => {
  const mockProps = {
    ...
    availableTransitions: mockStatusTransitions,
    visible: true,
    onClose: jest.fn(),
    onStatusChange: jest.fn(),
    onArchiveTrip: jest.fn(),
    onRestoreTrip: jest.fn(),
  ...
  describe('Modal Display', () => {
    it('should render when visible is true', () => {
      ...
      expect(screen.getByText('Manage trip lifecycle and status transitions')).toBeTruthy();
    });

    it('should not render when visible is false', () => {
      render(<TripStatusManager {...mockProps} visible={false} />);
  ...
  describe('Available Transitions', () => {
    ...
    it('should disable transitions when requirements not met', () => {
      const propsWithUnmetConditions = {
        ...mockProps,
        trip: { ...mockTrip, bookingsConfirmed: false, itineraryComplete: false }
      };
    ...
    it('should enable transitions when requirements are met', () => {
      ...
      expect(mockProps.onStatusChange).toHaveBeenCalledWith('CONFIRMED', 'Manual confirmation');
    });

    it('should filter transitions based on user permissions', () => {
      const collaboratorProps = { ...mockProps, userRole: 'COLLABORATOR' as const };
  ...
  describe('Automated Transitions', () => {
    it('should show upcoming automated transitions', () => {
      ...
      expect(screen.getByText('Will change to ONGOING on Feb 15, 2024')).toBeTruthy();
      expect(screen.getByText('Will change to COMPLETED on Feb 17, 2024')).toBeTruthy();
    });
    ...
    it('should allow disabling automated transitions', () => {
      ...
      expect(screen.getByText('Automatic transitions have been disabled')).toBeTruthy();
    });
  });
  ...
  describe('Status Validation', () => {
    ...
    it('should confirm status change when validation passes', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Change status from PLANNING to CONFIRMED?')).toBeTruthy();
      });
  ...
  describe('Trip Lifecycle Management', () => {
    ...
    it('should confirm trip archival', () => {
      ...
      expect(screen.getByText('Archive Trip')).toBeTruthy();
      expect(screen.getByText('Are you sure you want to archive this trip?')).toBeTruthy();
    ...
    it('should require confirmation for trip deletion', () => {
      ...
      expect(screen.getByPlaceholderText('Type trip name to confirm')).toBeTruthy();
    });
    ...
    it('should validate trip name before deletion', async () => {
      ...
      const confirmInput = screen.getByPlaceholderText('Type trip name to confirm');
  ...
  describe('Status Workflows', () => {
    ...
    it('should allow applying workflow templates', () => {
      ...
      expect(screen.getByText('This will set up automated transitions for business trips')).toBeTruthy();
    });

    it('should show workflow progress indicators', () => {
      const propsWithWorkflow = {
        ...mockProps,
        trip: { ...mockTrip, workflow: 'business', workflowStep: 2, totalSteps: 5 }
      };
  ...
  describe('Real-time Updates', () => {
    ...
    it('should update history with new events', () => {
      ...
      const newEvent: TripLifecycleEvent = {
        id: 'event-3',
        tripId: 'trip-1',
      ...
      rerender(<TripStatusManager {...mockProps} statusHistory={updatedHistory} />);
  ...
  describe('Accessibility', () => {
    ...
    it('should announce status changes', async () => {
      ...
      await waitFor(() => {
        const announcement = screen.getByLabelText('Status changed from PLANNING to CONFIRMED');
    ...
    it('should provide context for automated transitions', () => {
      ...
      const automatedInfo = screen.getByLabelText('Automatic transition to ONGOING scheduled for start date');
  ...
  describe('Error Handling', () => {
    it('should handle status change errors', async () => {
      const errorProps = {
        ...mockProps,
        onStatusChange: jest.fn().mockRejectedValue(new Error('Status change failed'))
      };
      ...
      await waitFor(() => {
        expect(screen.getByText('Failed to change status. Please try again.')).toBeTruthy();
      });
    });
    ...
    it('should handle archive errors', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Failed to archive trip. Please try again.')).toBeTruthy();
      });
    });
    ...
    it('should handle deletion errors', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Failed to delete trip. Please try again.')).toBeTruthy();
      });
    });
  });
  ...
  describe('Performance', () => {
    it('should handle large status histories efficiently', () => {
      ...
      const { container } = render(
        <TripStatusManager {...mockProps} statusHistory={largeHistory} />
      );
    ...
    it('should efficiently render status transitions', () => {
      ...
      const propsWithManyTransitions = { ...mockProps, availableTransitions: manyTransitions };
      
      const { container } = render(<TripStatusManager {...propsWithManyTransitions} />);
      expect(container).toBeTruthy();
    });
  });
});