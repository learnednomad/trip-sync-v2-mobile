...
import { Bookmark, Clock, MapPin, X } from '@/components/ui/icons';
...
const defaultTemplates: TripTemplate[] = [
  {
    ...
    description:
      'Professional trip with meetings, conferences, and networking events',
    tripType: 'BUSINESS',
    defaultDuration: 3,
  ...
  },
  {
    ...
    description:
      'Relaxing family trip with beach activities and quality time together',
    tripType: 'FAMILY',
    defaultDuration: 7,
...
export function TripTemplates({
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<TripTemplate | null>(
    null
  );
  ...
  const tripTypeOptions: { label: string; value: TripType | 'ALL' }[] = [
              ...
              <View className="space-y-6">
                    ...
                    <View className="flex-row space-x-2">
                      {selectedTemplate.destinations.map((destination) => (
                        <Button
                          key={destination}
                          variant={
                            customizations.destination === destination
                              ? 'default'
                              : 'outline'
                          }
                          size="sm"
                ...
                <View>
                  ...
                  <Text className="mt-1 text-xs text-gray-500">
                    Suggested: ${selectedTemplate.suggestedBudget.min} - $
                    {selectedTemplate.suggestedBudget.max}
                  </Text>
                </View>
                ...
                <View>
                  ...
                  <View className="flex-row flex-wrap">
                    {selectedTemplate.activities.map((activity, index) => (
                      <View
                        key={index}
                        className="mb-2 mr-2 rounded-full bg-gray-100 px-2 py-1"
                      >
                ...
                <View>
                  ...
                  <View className="flex-row flex-wrap">
                    {selectedTemplate.packingItems
                      .slice(0, 6)
                      .map((item, index) => (
                        <View
                          key={index}
                          className="mb-2 mr-2 rounded-full bg-blue-50 px-2 py-1"
                        >
                    ...
                    {selectedTemplate.packingItems.length > 6 && (
                      <View className="rounded-full bg-gray-100 px-2 py-1">
                        <Text className="text-xs text-gray-600">
                          +{selectedTemplate.packingItems.length - 6} more
                        </Text>
                      </View>
                    )}
                  </View>
            ...
            <ScrollView className="flex-1 px-4 py-6">
              <View className="flex-row flex-wrap justify-between">
                {filteredTemplates.map((template) => (
                  <View
                    key={template.id}
                    className="mb-4 w-[48%] rounded-lg border border-gray-200 bg-white shadow-sm"
                  >
                    {/* Template Card */}
                    <View className="p-4">
                      <View className="mb-2 flex-row items-center justify-between">
                        <View
                          className={`rounded-full px-2 py-1 ${getTripTypeColor(template.tripType)}`}
                        >
