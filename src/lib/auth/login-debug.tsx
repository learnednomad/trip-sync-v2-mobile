    ...
    try {
      ...
      if (data.data) {
        ...
        if (data.data.tokens) {
          ...
          console.log('  - access token length:', data.data.tokens.access?.length);
          console.log('  - refresh token length:', data.data.tokens.refresh?.length);
        }
      }
      
      // Try to manually store tokens if they exist
      if (data.success && data.data?.tokens?.access && data.data?.tokens?.refresh) {
        const tokenData = {
